...
async function getVerificationData(url, email) {
  const tokenName = `POLYBOT CLI ${os.platform()}-${os.arch()} ${pkg.version} (${os.hostname()})`
  const data = JSON.stringify({email, tokenName})
  const res = await fetch(`${url}/users/registration`, {
    method: 'POST',
    headers: {
...
async function verify(url, email, verificationToken) {
  ...
  const res = await fetch(`${url}/users/registration/verify?${stringifyQuery(query)}`, {
